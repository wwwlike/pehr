                ...
                <div class="ibox float-e-margins">
                    <div class="ibox-title manage-head-con padding-left">
                        ...
                        <div class="ibox-tools ">
                            <a href="/schoolPay/input" target="_self" class="btn btn-primary btn-xs">创建缴费</a>
                        </div>
                    </div>
                        ...
                        <table class="table table-striped table-bordered table-hover dataTables-example">
                            ...
                            <tbody>
                                ...
                                <tr class="gradeU" id="${bean.id}">
                                    <td>${bean.title}</td>
                                    <td class="center"><citms:customLabel key="PAY_TYPE" value="${bean.type}"></citms:customLabel>  </td>
                                    <td class="center"><citms:customLabel key="PAY_STATUS" value="${bean.status}"/></td>
                                    <td><citms:nameShow id="${bean.schoolId}" type="school"></citms:nameShow></td>
                                    <td>${bean.payDate}</td>
                                    <td>${bean.pay}</td>
                                    <td class="center">
	                                    <c:if test="${bean.status=='1'||bean.id=='2'}">
	                                      <a href="/schoolPay/view?id=${bean.id}" target="_self" class="btn btn-warning btn-xs">查看</a>
	                                    </c:if>
	                                    <c:if test="${bean.status=='0'}">
	                                     	<a href="/schoolPay/input?id=${bean.id}"  target="_self"  class="btn btn-warning btn-xs">
	                                   			发布
	                                    	</a>
	                                    	<button onclick="deleteEntity('${bean.id}')"   class="btn btn-warning btn-xs">
	                                   			删除
	                                    	</button>
	                                 	</c:if>
                                    </td>
